...
The **DOM (Document Object Model)** is JavaScript's interface to the page — a live, tree-shaped representation where every element is a node with a parent, siblings, and children. (See Part 3 for the full picture of why the DOM matters.)
...
**Fetch API** — the standard way to call HTTP from the browser (covered in Part 3). Quick form:
...
(Node and npm are deep-dived in Part 3. This module focuses on what you actually build with them.)
...
(PHP is deep-dived in Part 3. The basics: `.php` files mix HTML and code in `<?php ... ?>` tags; the server runs the code per request.)
...
(SQL is deep-dived in Part 3.) A relational database stores **entities** (users, products, orders) in **tables** — each row is one entity, each column is one attribute. The four core operations form the acronym **CRUD**: Create, Retrieve, Update, Delete.
...
(React is deep-dived in Part 3.) The mental shift to remember: **declarative, not imperative** — you describe what the UI should look like for a given state and React diffs against the DOM.
...
Build small projects end-to-end rather than reading endlessly — the only way to internalize this stack is to ship something messy and then improve it.
